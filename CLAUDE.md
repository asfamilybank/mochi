...
- **`.fullSizeContentView` 不能拿来做"工具栏背景透明"**：加上之后网页内容会铺到标题栏底下，页面顶部的图文直接穿到交通灯和工具栏按钮上（实测：站点 logo 和标题压在红黄绿按钮上），不是 Safari 那种干净的材质叠加。Safari 能那样是因为它自己管理内容内边距，不是单靠这个 styleMask。
