...
- **CI runner 的 macOS 比开发机新**（实测 runner 26.6.2 / 25G83，本机 26.5.2 / 25F84），而 SF Symbols 的 `alignmentRect` 度量在这两个小版本之间变了：15pt 的 `arrow.clockwise` 在 26.5 上是 10.5、26.6 上是 11.0（13pt/17pt 两版一致）。把自绘字形的计算值和**活的系统符号**钉成严格相等的断言因此会在 CI 上单独红一档——`GhostGlyphTests` 已改成允许半点（一个量化步长）偏差，后续影响记在 [#55](https://github.com/asfamilybank/mochi/issues/55)。新写这类「跟系统实物对照」的测试时默认留一个量化步长的余量，别拿本机实测值当跨版本常量。
